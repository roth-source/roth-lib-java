...
public abstract class Config
{
	...
	public boolean hasSerializer(Class<?> klass)
	{
	...
	public Config setTimeFormat(String timeFormat)
	{
		this.timeFormat = timeFormat;
		for(Serializer<?> serializer : serializerMap.values())
		{
			if(serializer instanceof Temporal)
			{
				((Temporal) serializer).setTimeFormat(timeFormat);
			}
		}
		for(Deserializer<?> deserializer : deserializerMap.values())
		{
			if(deserializer instanceof Temporal)
			{
				((Temporal) deserializer).setTimeFormat(timeFormat);
			}
		}
		return this;
	}
	
	public abstract Config setPrettyPrinting(boolean prettyPrinting);
	public abstract Config setSerializeNulls(boolean serializeNulls);
	public abstract <T> Config setSerializer(Class<T> klass, Serializer<T> serializer);
	public abstract <T> Config setDeserializer(Class<T> klass, Deserializer<T> deserializer);
